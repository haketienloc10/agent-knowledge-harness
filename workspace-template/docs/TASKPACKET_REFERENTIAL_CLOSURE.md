...
Rule này áp dụng chung cho prior-turn text, media, file/attachment, bảng, log/snippet, tool result, external observation, generated artifact, tracked Work Item hoặc bất kỳ object/context nào nằm ngoài current packet.
...
- tracked Work Item dossier khi qiqi_delegate mount Work Items root và TaskPacket truyền trusted fact `work_item_path=<absolute dossier path>; id=<canonical-id>; revision=<n>`;
...
Work Item locator là **continuity/provenance locator**, không phải semantic escape hatch. Child MAY dereference dossier cho durable current-state context, nhưng TaskPacket vẫn phải chứa objective/scope/acceptance và mọi assignment semantics material cần để thực thi turn.
...
- Work Item không có explicit mounted absolute locator, Knowledge hoặc sibling state để reconstruct missing task meaning;
...
1. Xác định các task-material semantics hiện tại, kể cả semantics đến từ turn/artifact/tool output/Work Item trước đó.
2. Tìm các reference mà meaning phụ thuộc vào referent ngoài candidate packet.
3. Với mỗi referent, xác định chosen START/RESUME mode và configured resource mounts có bảo đảm child access referent hay không.
4. Nếu **có**, identify referent đủ unambiguous và vẫn preserve material provenance/coverage trong packet khi chúng ảnh hưởng confidence hoặc acceptance. Tracked Work Item dùng absolute `work_item_path` + canonical `id` + delegated `revision`.
...
10. Sau closure, áp dụng minimality bình thường: không forward toàn conversation/artifact/Work Item nếu một semantic distillation nhỏ hơn đã đủ cho assignment.

## Mapping vào TaskPacket hiện có

Referential closure **không yêu cầu một TaskPacket top-level field mới**. Map theo semantic role:

- observed/external premise child MAY rely on → `context.trusted_facts[]` + `source`;
- tracked Work Item continuity/provenance → `context.trusted_facts[]` với `work_item_path=<absolute dossier path>; id=<canonical-id>; revision=<n>`;
...
Tracked Work Item không đổi nguyên tắc này: packet có thể mang locator để child đọc current durable dossier, nhưng một fresh START không được bỏ objective/scope/acceptance chỉ vì dossier đã mounted.
...
- cấm child đọc explicitly mounted tracked Work Item bằng absolute locator;
...
11. tracked Work Item absolute locator child-visible nhưng packet vẫn chứa self-sufficient objective/scope/acceptance.

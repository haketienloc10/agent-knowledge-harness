...
QiQi sở hữu task semantics và các quyết định orchestration. Execution agent sở hữu
repo-local work trong scope được giao. MCP sở hữu execution lifecycle và result
handoff phía sau public tool contract.
...
Chi tiết orchestration, START/RESUME, delegation waves, result handoff, failure và
reporting được định nghĩa trong `AGENTS.md`; `identity.md` chỉ giữ danh tính, trách
nhiệm cấp cao và hard boundaries của QiQi.

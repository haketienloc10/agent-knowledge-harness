...
install_skill() {
  local client="$1"
  local root="$2"
  local target="$root/knowledge-distill"
  ...
  if [[ -d "$target" && ! -f "$marker" ]]; then
    ...
    printf 'ERROR: %s skill `knowledge-distill` already exists and is not managed by this harness: %s\n' \
      "$client" "$target" >&2
    printf 'Move/remove that skill explicitly, then rerun installer.\n' >&2
    return 78
  fi

  temp_parent="$(mktemp -d "$root/.knowledge-distill.XXXXXX")"
  temp="$temp_parent/knowledge-distill"

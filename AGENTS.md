...
- Mọi repo-local work từ QiQi đi qua đúng một synchronous MCP tool:
  `delegate_repo_task`.
- MCP tool chỉ trả terminal structured result cùng native agent session ID; không
  expose progress, status, wait hoặc transcript.
...
6. Route config sở hữu command/model/flags; MCP chỉ build argv, START/RESUME,
   extract native session ID và normalize result.
7. Native resume phải kiểm tra identity: session ID trả về phải khớp ID yêu cầu.
8. Tool result contract phải đủ cho QiQi reconcile mà không tự vào repo kiểm tra.
9. Nếu thay đổi artifact bắt buộc, cập nhật checker và tài liệu setup cùng lúc.
